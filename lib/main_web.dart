...
class DB {
  ...
  static dynamic _json(String k, dynamic def) {
    try {
      final v = _s[k];
      return v == null ? def : jsonDecode(v);
    } catch (_) {
      return def;
    }
  }
  ...
  static void saveMoviesCache(String ch, List<dynamic> l) {
    try {
      _s['movies_$ch'] = jsonEncode(l);
    } catch (_) {}
  }
...
class _WebHomeState extends State<WebHome> {
    ...
    showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(hintText: 'الصق رابط القناة أو @اليوزر'),
        ),
        actions: [
        ...
        actions: [
          Padding(
            ...
            child: FilledButton.icon(
              onPressed: () => html.window.open(tgLink, '_blank'),
              icon: const Icon(Icons.download, size: 16),
              label: const Text('المشاهدة على التطبيق', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                          ...
                          if (cont.isNotEmpty && q.isEmpty) ...[
                            Padding(
                              ...
                              child: Align(
                                ...
                                child: Text('▶️ متابعة المشاهدة', style: TextStyle(color: accent, fontWeight: FontWeight.bold)),
                              ),
                            ),
                            SizedBox(
                              height: 190,
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        const Padding(
                                          ...
                                          child: Text(
                                            'تعذر الاتصال — تأكد من تفعيل CORS على السيرفر أو أضف قناة أولاً',
  ...
  Widget _chip(String id, String label) => Padding(
    ...
    child: ChoiceChip(
      ...
      selected: scope == id,
      onSelected: (_) => setState(() => scope = id),
  ...
  Widget _grid(List<WMovie> list, Map<String, List<WMovie>> groups) => list.isEmpty
      ...
      : GridView.builder(
          ...
          itemBuilder: (_, i) => _card(list[i], groups),
        );
        ...
        child: Stack(
          ...
          children: [
            Image.network(
              m.poster,
              fit: BoxFit.cover,
              errorBuilder: (_, __, ___) => const Center(child: Icon(Icons.movie, size: 46, color: Colors.white24)),
            ),
            const DecoratedBox(
            ...
            Positioned(
              ...
              right: 6,
              bottom: 6,
              child: Text(m.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.white)),
            ),
            ...
            if (g != null && g.length > 1)
              Positioned(
                ...
                child: Container(
                  ...
                  child: Text('سلسلة ${g.length}', style: const TextStyle(fontSize: 9, color: Colors.white)),
                ),
              ),
            ...
            if (pos > 60)
              Positioned(
                ...
                child: Directionality(
                  ...
                  child: LinearProgressIndicator(minHeight: 4, value: 0.3, backgroundColor: Colors.black54, valueColor: const AlwaysStoppedAnimation(accent)),
                ),
              ),
          ],
        ),
      ...
      itemBuilder: (_, i) {
        ...
        return ListTile(
          leading: ClipRRect(
            ...
            child: Image.network(m.poster, width: 55, height: 80, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.movie)),
          ),
                ...
                children: [
                  ClipRRect(
                    ...
                    child: Image.network(
                      ...
                      fit: BoxFit.cover,
                      errorBuilder: (_, __, ___) => const Icon(Icons.movie, size: 80, color: Colors.white24),
                    ),
                  ),
                  Positioned(
                    top: 8,
                  ...
                  Positioned(
                    ...
                    right: 8,
                    child: IconButton(
                      icon: Icon(DB.isFav(m.id) ? Icons.favorite : Icons.favorite_border, color: Colors.red),
                      onPressed: () {
                        DB.toggleFav(m.id);
                        Navigator.pop(c);
                        _details(m, g);
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(
                      width: double.infinity,
                      child: FilledButton.icon(
                        style: FilledButton.styleFrom(backgroundColor: accent, foregroundColor: Colors.black, minimumSize: const Size.fromHeight(48)),
                        onPressed: () {
                          Navigator.pop(c);
                          Navigator.push(context, MaterialPageRoute(builder: (_) => WebPlayer(m: m)));
                        },
                      ...
                      Wrap(
                        ...
                        children: m.alts
                            .map((a) => ActionChip(
                              backgroundColor: bg,
                              side: BorderSide.none,
                              label: Text(a['q'] ?? 'جودة', style: const TextStyle(fontSize: 11, color: Colors.white)),
                              onPressed: () {
                                Navigator.pop(c);
                                Navigator.push(context, MaterialPageRoute(builder: (_) => WebPlayer(m: m, url: a['url'], q: a['q'])));
                              },
                            ))
                            .toList(),
                      ),
                    ...
                    if (g != null && g.length > 1) ...[
                      ...
                      ...g.map((p) => ListTile(
                        ...
                        leading: ClipRRect(
                          ...
                          child: Image.network(p.poster, width: 45, height: 62, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.movie, size: 24)),
                        ),
                        ...
                        onTap: () {
                          Navigator.pop(c);
                          Navigator.push(context, MaterialPageRoute(builder: (_) => WebPlayer(m: p)));
                        },
                      )),
                    ],
                  ],
  ...
  Widget _tag(String t) => Container(
    ...
    child: Text(t, style: const TextStyle(fontSize: 10, color: accent)),
  );
}
...
class _WebPlayerState extends State<WebPlayer> {
  ...
  void initState() {
    ...
    _el.onTimeUpdate.listen((_) {
      DB.savePos(widget.m.id, _el.currentTime.toInt());
    });
    _el.onEnded.listen((_) {
      DB.savePos(widget.m.id, 0);
    });
  ...
  void _switch(String url) {
    ...
    _curUrl = url;
    _el.src = url;
    _el.onLoadedMetadata.first.then((_) {
      _el.currentTime = t;
      _el.play();
    });
    setState(() {});
